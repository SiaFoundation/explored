...
func addTransactions(tx *txn, bid types.BlockID, txns []types.Transaction) (map[types.TransactionID]bool, error) {
	...
	for i, txn := range txns {
		...
		if !exist {
			if err := insertTransactionStmt.QueryRow(encode(txnID)).Scan(&dbID); err != nil {
...
func updateStateTree(tx *txn, changes []explorer.TreeNodeUpdate) error {
	stmt, err := tx.Prepare(`INSERT INTO state_tree (row_id, col_id, value) VALUES($1, $2, $3) ON CONFLICT (row_id, col_id) DO UPDATE SET value=EXCLUDED.value;`)
	if err != nil {
		return fmt.Errorf("failed to prepare statement: %w", err)
	}
	defer stmt.Close()
...
func resetChainState(tx *txn, log *zap.Logger, target int64) error {
	...
	for _, name := range names {
		...
		if _, err := tx.Exec(fmt.Sprintf(`DROP TABLE IF EXISTS %s CASCADE`, name)); err != nil {
...
func (s *Store) ResetChainState() error {
	if err := s.transaction(func(tx *txn) error {
		// foreign-key constraints in postgres are deferred via SET CONSTRAINTS,
		// but DROP TABLE bypasses them. CASCADE on DROP is enough.
		if err := resetChainState(tx, s.log, int64(len(migrations)+1)); err != nil {
	...
	if _, err := s.pool.Exec(context.Background(), `VACUUM`); err != nil {
